...
Incluye fechas de última recepción y última salida, botón flotante para subir,
y ahora edición de movimientos.
"""
...
- **Fechas de última recepción y última salida** en la tabla principal
- **Botón flotante para subir** al inicio de la página
- **Edición de movimientos** (tipo, cantidad, notas)
...
def get_last_reception_date(product_id):
    \"\"\"Fecha del último movimiento de entrada (IN) para un producto, si el stock actual > 0.\"\"\"
...
def get_last_dispatch_date(product_id):
    \"\"\"Fecha del último movimiento de salida (OUT) para un producto.\"\"\"
...
from utils import update_inventory, revert_inventory, get_current_stock, get_last_reception_date, get_last_dispatch_date
from datetime import datetime
import click
import os
import re
...
def edit_transaction(id):
    ...
    # Fijar el producto como solo lectura (deshabilitado)
    form.product_id.choices = [(trans.product_id, trans.product.name)]
    ...
    if form.validate_on_submit():
        ...
        if new_type == 'OUT':
            # Calcular stock actual después de revertir
            current_stock = get_current_stock(old_product_id)
            if current_stock < new_quantity:
                # Restaurar el movimiento antiguo para no dejar inconsistencia
        ...
        try:
            ...
            # Si falla, intentar restaurar el antiguo (ya revertido, pero hay que volver a aplicar)
            update_inventory(old_product_id, old_quantity, old_type)
            db.session.rollback()
...
<table class="table table-bordered table-hover">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{ item.stock }}</td>
            <td>
                {% if item.last_reception %}
                    {{ item.last_reception.strftime('%d/%m/%Y %H:%M') }}
                {% else %}
                    —
                {% endif %}
            </td>
            <td>
                {% if item.last_dispatch %}
                    {{ item.last_dispatch.strftime('%d/%m/%Y %H:%M') }}
                {% else %}
                    —
                {% endif %}
            </td>
    ...
    <div class="card-body">
        ...
        <form method="POST">
            ...
            <div class="mb-3">
                ...
                {{ form.product_id(class="form-select", disabled=True) }}
            </div>
            <div class="mb-3">{{ form.transaction_type.label(class="form-label") }}{{ form.transaction_type(class="form-select") }}</div>
            <div class="mb-3">{{ form.quantity.label(class="form-label") }}{{ form.quantity(class="form-control") }}</div>
            <div class="mb-3">{{ form.notes.label(class="form-label") }}{{ form.notes(class="form-control") }}</div>
...
<table class="table table-striped">
    ...
    <tbody>
        ...
        {% else %}
        <tr><td colspan="2" class="text-center">No hay productos en papelera.{% endfor %}
    </tbody>
</table>
{% endblock %}
""")
...
if __name__ == '__main__':
    ...
    print("✅ Fechas de última recepción y última salida en tabla principal")
    print("✅ Botón flotante para subir")
    print("✅ Edición de movimientos (tipo, cantidad, notas)")
